 ...
 * Agora considera o fator de transito real (trafficFactor) para projecao mais precisa.
 */
function calculateForecast(vehicle, nowMs, horizon = 30, stepSec = 2, trafficFactor) {
  const { meta, baseSpeed, offsetKm } = dynamics(vehicle);
  const effectiveSpeed = trafficFactor != null ? baseSpeed * Math.min(trafficFactor, 1.2) : baseSpeed;
  const elapsedH = (nowMs - EPOCH) / 3600000;
  const d0 = offsetKm + effectiveSpeed * elapsedH;
  const points = [];
  for (let t = 0; t <= horizon; t += stepSec) {
    const d = d0 + effectiveSpeed * (t / 3600);
    const p = pointAtDistance(meta, d);
