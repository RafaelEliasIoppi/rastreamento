...
function calculatePosition(vehicle, nowMs) {
  ...
  const d = offsetKm + baseSpeed * elapsedH;
  const p = pointAtDistance(meta, d);

  // Velocidade reportada com leve variacao (a posicao usa baseSpeed, deterministico).
  const wobble = Math.sin((nowMs / 1000) * 0.2 + vehicle.seed) * 6;
  const shownSpeed = Math.max(12, baseSpeed + wobble);
  ...
  return {
    ...
    speed: Math.round(shownSpeed * 10) / 10,
    status: 'normal',
    lastUpdate: nowMs,
  };
}
